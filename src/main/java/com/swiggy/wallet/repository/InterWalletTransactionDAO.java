...
public interface InterWalletTransactionDAO extends JpaRepository<InterWalletTransaction, Integer> {
    ...
    @Query("SELECT t FROM InterWalletTransaction t INNER JOIN t.deposit where (t.sender = ?1 or t.receiver = ?1) and (t.deposit.timestamp BETWEEN ?2 AND ?3) and (t.withdrawal.timestamp BETWEEN ?2 AND ?3)")

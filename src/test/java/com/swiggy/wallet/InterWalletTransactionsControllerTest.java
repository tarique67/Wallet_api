...
public class InterWalletTransactionsControllerTest {
    ...
    void expectTransactionSuccessful() throws Exception {
        ...
        when(transactionsService.transact(interWalletTransactionRequestModel)).thenReturn(expected);
        ...
        verify(transactionsService, times(1)).transact(interWalletTransactionRequestModel);
    }
    ...
    void expectSameWalletExceptionIfTransferringToSameAccount() throws Exception {
        ...
        String requestJson = objectMapper.writeValueAsString(interWalletTransactionRequestModel);
        when(transactionsService.transact(interWalletTransactionRequestModel)).thenThrow(new SameWalletsForTransactionException(WALLETS_SAME_IN_TRANSACTION));
    ...
    void expectAllTransactionsOfUser() throws Exception {
        ...
        IntraWalletTransaction withdrawal = new IntraWalletTransaction(new Money(100, Currency.INR), IntraWalletTransactionType.WITHDRAW,new Wallet(), LocalDateTime.now());
        when(transactionsService.allTransactions()).thenReturn(new TransactionsResponseModel(Arrays.asList(new InterWalletTransactionResponseModel(1, "sender",1,"receiver",2, deposit, withdrawal, new Money(0, Currency.INR))), new ArrayList<>()));

        mockMvc.perform(get("/api/v1/transactions")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestJson))
                .andExpect(status().isOk());
        verify(transactionsService, times(1)).allTransactions();
    }
    ...
    void expectUnauthorizedForAllTransactions() throws Exception {
        ...
        verify(transactionsService, times(0)).allTransactions();
    }
    ...
    public void expectAllTransactionsDateBased() throws Exception {
        ...
        when(transactionsService.allTransactionsDateBased(startDate, endDate)).thenReturn(new TransactionsResponseModel(mockResponse, new ArrayList<>()));
        ...
        mockMvc.perform(get("/api/v1/transactions")
                ...
                .andExpect(jsonPath("$.interWalletTransactions[0].sender").value("sender"))
                .andExpect(jsonPath("$.interWalletTransactions[0].receiver").value("receiver1"))
                .andExpect(jsonPath("$.interWalletTransactions[0].deposit.money.amount").value(100))
                .andExpect(jsonPath("$.interWalletTransactions[1].sender").value("sender"))
                .andExpect(jsonPath("$.interWalletTransactions[1].receiver").value("receiver2"))
                .andExpect(jsonPath("$.interWalletTransactions[1].deposit.money.amount").value(200));
        verify(transactionsService, times(1)).allTransactionsDateBased(startDate,endDate);
    }
    ...
    void expectUnauthorizedForAllTransactionsDateBased() throws Exception {
        ...
        IntraWalletTransaction deposit = new IntraWalletTransaction(new Money(100, Currency.INR), IntraWalletTransactionType.DEPOSIT,new Wallet(), LocalDateTime.now());
        IntraWalletTransaction withdrawal = new IntraWalletTransaction(new Money(100, Currency.INR), IntraWalletTransactionType.WITHDRAW,new Wallet(), LocalDateTime.now());
        when(transactionsService.allTransactionsDateBased(LocalDate.now(), LocalDate.now())).thenReturn(new TransactionsResponseModel(Arrays.asList(new InterWalletTransactionResponseModel(1,"sender1", 1, "receiver1",2, deposit, withdrawal, new Money(0, Currency.INR))), new ArrayList<>()));

        mockMvc.perform(get("/api/v1/transactions")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestJson))
                .andExpect(status().isUnauthorized());
        verify(transactionsService, times(0)).allTransactionsDateBased(LocalDate.now(), LocalDate.now());
    }
}
